...
export default function LockScreen({ onUnlock }: LockScreenProps) {
  ...
  const envPasswordsRaw = (process.env.VALID_PASSWORDS || process.env.NEXT_PUBLIC_VALID_PASSWORDS || "").toString();
        ...
        <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-8">
          ...
          <div className="text-center mb-8">
            <div className="flex justify-center mb-4">
              <img className="h-16 w-auto logo-light dark:hidden" src="/logo/surge-icon-rec-light.svg" alt="Surge Logo" />
              <img className="h-16 w-auto logo-dark hidden dark:block" src="/logo/surge-icon-rec-dark.svg" alt="Surge Logo" />
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#f4431b] focus:border-transparent bg-white dark:bg-[#0d1111] text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 transition-all duration-200" placeholder="Enter access password" required disabled={isLoading} />
            </div>
            ...
            <button type="submit" disabled={isLoading || !password.trim()} className="auth-btn">
              {isLoading ? (
