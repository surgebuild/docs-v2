...
q.requiredSats            // 10472n   <- send this
```

Max LTV is per-market, so `marketId` decides the answer - the same $10 needs ~1/8 more collateral at 70% LTV than at 80%. For a fully-repaid position you intend to switch, pass the **target** market; while debt is live the market is locked and quoting a different one throws.
...
```ts
const { inLiquidation, liquidated, type, at } = await session.getLiquidation(positionId);
```
...
| `AMOUNT_BELOW_MINIMUM` | The borrow is under the `$5` minimum (`MIN_BORROW_USD`) - thrown before any signature |
...
| `INVALID_FEE_RATE` | `feeRate` is zero, negative, non-finite, or above the 1000 sat/vB ceiling - check the units, it is sat/vB not sats |
| `POSITION_IN_LIQUIDATION` | The collateral is being seized - no credit action applies |
| `POSITION_EXPIRED` | The term ended (`isActive: false`) and collateral remains - `extendCreditLine` is available to renew it, though the contract can still refuse |
...
**When a position ends.** `position.isActive` is the actionability signal: it goes false when the term ends, when the line is repaid and closed, or when a liquidation settles. Drawing against any of those is refused; repay, withdraw, and `extendCreditLine` stay available, and `extendCreditLine` is the way to renew. Available is not the same as guaranteed: the contract still decides whether a given position can be extended. Whether you get `POSITION_EXPIRED` or `POSITION_CLOSED` depends on whether collateral remains to renew against.
...
```ts
const p = await session.getPosition(positionId);

if (p.inLiquidation) {
  // Nothing to do - the collateral is being seized. session.getLiquidation() for detail.
} else if (p.isActive) {
