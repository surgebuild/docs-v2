...
The onchain State maintains the **single source of truth** on EVM for:
...
- **Bitcoin Attestations**: Proof-of-inclusion for UTXOs and Taproot spends are submitted to execution state, ensuring the ledger mirrors real Bitcoin state.
...
- Oracle's medianized price point from multiple sources in execution state is used for CR enforcement.  
