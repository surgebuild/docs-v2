...
When a vault’s collateral ratio (CR) drops below its per market safety threshold, the **liquidation path** activates automatically.  BTC is spent to the DCN, which starts and dutch auction process. The procceeds from the auction return to the market from which the credit line was liquidated.  
