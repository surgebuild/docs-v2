...
Surge is a Bitcoin native lending platform that's transparent, programmable, and designed to serve grassroots Bitcoiners. BTC is always locked in Taproot outputs with [MAST](https://bitcoinops.org/en/topics/mast/) based spend conditions, while execution environments mirror dvault conditions and coordinate stablecoin liquidity across chains.
...
- **Signer Epochs** - Track active signer sets, validate MPC signatures, and coordinate epoch rotation.
- **Collateral Monitoring** - Continuously monitor vault health via oracles. Triggers liquidation when CR < MinCR.
- **Stability Pool** - Absorbs undercollateralized debt and distributes discounted BTC to depositors.
- **PSBT Coordination** - During micro lot liquidations, the Execution Program constructs Partially Signed Bitcoin Transactions ([PSBTs](https://learnmeabitcoin.com/technical/transaction/psbt/)) representing each tranche of BTC collateral.
...
Execution runs on existing EVM-compatible chains with deterministic programs for consistent state replication.
...
- **Deterministic Reconciliation** - Ensures stablecoin supply always matches to BTC collateral locked in dVaults.
