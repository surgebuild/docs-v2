...
# 🔐 Wallet Custody & Secure Enclave

User wallets are designed to be **self-custodial** from the ground up.  
Each user’s keys are generated inside hardware-isolated environment called a **Trusted Execution Environment (TEE)**, the institutional grade security model used in Coinbase’s enclave based wallet infrastructure. 

The TEE runs as an isolated runtime on our infrastructure. Even our engineers, administrators, or the host OS cannot read, export, or alter a user’s key.
...
- Backend re-prompts for OTP → sends an export command to the enclave.  
- The enclave decrypts and returns the key encrypted.  
- This encrypted blob is the user’s portable backup.  
...
- Proves it’s running the **approved wallet code**.  
...
Enclaves can’t be modified once live. To ship updates or bug fixes:
