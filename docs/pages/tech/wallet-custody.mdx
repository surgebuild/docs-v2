...
# 🔐 Wallet Custody & Trusted Execution Enclave

The Surge wallet custody layer implements a **self-custodial design** that isolates all cryptographic key operations within a hardware backed **Trusted Execution Environment (TEE)**.  
...
Once instantiated, its memory and code are cryptographically measured and isolated — even Surge operators, administrators, or the host kernel cannot read, export, or modify a user’s private key.
...
| **User Sovereignty** | Signing only under valid JWT session; Surge cannot initiate transactions. |
