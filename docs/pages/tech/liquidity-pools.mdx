---
title: ⚙️ Liquidity Management & Stability Pools
description: Deterministic routing and liquidation flow for stablecoin settlement and BTC recovery.
---

# ⚙️ Liquidity Management & Stability Pools

Surge’s liquidity layer connects **cross-chain stablecoin routing** with **on-chain collateral management**.  
While stablecoins move freely across networks, all liquidation and recovery activity resolves through the **canonical Stability Pool** - the protocol’s equilibrium mechanism for BTC-backed solvency.
...
- **Deterministic Settlement** Transfers finalize through signed attestations verifiable on-chain, ensuring mint/burn parity and preventing double liquidity issuance.
...
## 2. Canonical Stability Pool

The **Stability Pool** is the canonical clearinghouse for all liquidation and recovery events.  
It absorbs undercollateralized debt using stablecoin liquidity and receives discounted BTC tranches in return.

| Component | Function |
|------------|-----------|
| **Stability Pool** | Primary settlement contract for liquidation and BTC recovery |
...
The Stability Pool thus acts as **systemic insurance**, guaranteeing that bad debt is cleared instantly and without external liquidator competition.
...
Each iteration cancels a fraction of the vault’s debt and transfers a corresponding BTC slice to the Stability Pool.
...
| \( s \) | Stability pool discount | 0.05 |
...
This defines the amount of stablecoin the Stability Pool must provide to restore CR ≈ RecCR.
...
Vault solvency is restored, and liquidation proceeds are distributed to the Stability Pool.
...
| BTC to Stability Pool | – | 0.0499 |
| Post CR | – | 151.7% |
| Post LTV | – | 65.9% |

The system cancels ~$8.1k of debt, sends 0.0499 BTC (discounted 5%) to the Stability Pool, and restores the vault to its recovery CR target (~1.7× collateralization).
...
3. **Stability Pool Settlement** - Stablecoins are burned; BTC is sent at discount.  
...
- [dVaults](/tech/dvaults) - Script-level spend paths  
- [Repayment](/tech/repayment) - Normal closure logic  
- [Oracle System](/tech/oracle-system) - Medianized BTC/USD feed
