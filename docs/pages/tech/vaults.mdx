...
- **Key Paths** - The standard way to move BTC, co-signed by the user and Surge’s current signer set using MuSig2
- **Script Paths** - Extra branches in the vault’s tree that only open under specific conditions:
...
- The **key path** (MuSig2 aggregate of user + signer epoch) handles the common “repayment” flow.
...
4. **Incremental Liquidations** — If `CR < MinCR`, signers spend via the liquidation leaf. The spend MUST produce:
   - **Stability Pool purchase** — discounted BTC to the Stability Pool, and
