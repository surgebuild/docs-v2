...
- **Key Path** - The repayment/close path, co-signed by the user and Surge’s signer set using **Lin22 (threshold Schnorr)** via Coinbase cb-mpc.
...
- **Script leaves** enforce exceptional paths (liquidation, timelock, migration).
...
5. **Repayment/Close** - Upon debt repayment, signers co-sign the key-path spend, moving BTC per user instructions.
