...
- **Taproot** commits multiple possible spending rules inside a single address.
...
A dVault locks BTC in a Taproot script address with multiple **spend paths**:
...
- **Script Paths** - additional branches in the vault’s taptree that open under specific conditions:
...
- Each vault commits a **taptree** of spend leaves (tapscripts). Only the executed leaf is revealed during spend; unused branches remain hidden.
- The **repayment path** uses an **aggregate Lin22 Schnorr key** (`cb-mpc(P_user, P_signer_epoch)`) for the standard close flow.
- **Script leaves** define exceptional paths (liquidation, CR checks, timelock, etc.,).
...
- **Liquidation** - If CR < MinCR, signers spend via a **CSV gated** liquidation leaf. The spend creates outputs:
   ...
   - Re-locks change to a new dVault under the borrower’s control.
