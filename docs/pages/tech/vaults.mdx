...
- **Taproot** commits multiple spend rules into a single address.
- **MAST (Merkelized Alternative Script Tree)** organizes those rules into a hidden tree: only the rule used is revealed when spending.
...
A dVault locks BTC in a Taproot output script address with multiple **spend paths**:
